...
def generate_voxel_32(
    net, latent_flow_model, clip_model, args, num_figs_per_query=5, prefix="a"
):
    net.eval()
    ...
    with torch.no_grad():
        ...
        for text_in in tqdm(total_text_query):
            ...
            decoder_embs = latent_flow_model.sample(
                num_figs,
                noise=noise,
                cond_inputs=text_features.repeat(num_figs, 1).float(),
            )
        ...
        for text_in in tqdm(total_text_query):
            ...
            for voxel_in in voxels_out:
                out_file = os.path.join(
                    save_path, text_in + "_" + str(voxel_num) + ".png"
                )
...
def get_local_parser_test(mode="args"):
    ...
    parser.add_argument(
        ...
        type=str,
        default="./exps/classifier/checkpoints/best.pt",
        metavar="N",
        help="what is the classifier checkpoint for FID, Acc and Stuff",
    )
    parser.add_argument(
        "--checkpoint_nf",
        type=str,
        default="best",
        metavar="N",
        help="what is the checkpoint for nf",
    )
    parser.add_argument(
        "--prefix", type=str, default="a", metavar="N", help="add or remove"
    )
    parser.add_argument(
        "--post_dataset",
        type=str,
        default=None,
        help="if want to use diff dataset during post",
    )
    parser.add_argument(
        "--checkpoint_dir_base",
        type=str,
    ...
    parser.add_argument(
        "--output_dir", type=str, default="./exps/output_dir", help="output dir"
    )
    parser.add_argument(
        "--checkpoint_dir_prior", type=str, default=None, help="Checkpoint for prior"
    )
...
def main():
    ...
    if args.experiment_mode not in [
        ...
        test_dataloader, total_shapes_test, test_dataset = get_dataloader(
            args, split="test", dataset_flag=True
        )
    ...
    if args.experiment_mode == "fid_cal":
        ...
        generated_voxels, _ = generate_voxel_32(
        ...
        activations, pred_labels = classifier.get_activations(
