...
def get_cond_embeddings(
    data, input_type: str, device, autoencoder: Autoencoder, clip_model: CLIP
):
...
def get_condition_embeddings_2(
    input_type: str,
    device,
    autoencoder: Autoencoder,
    clip_model: CLIP,
    dataloader: shapenet_dataset.Shapes3dDataset,
    ...
    """
    # TODO: It probably makes more sense to directly consume the dataset and not the DataLoader.

    autoencoder.eval()
    clip_model.eval()
    ds = IterableWrapper(dataloader, deepcopy=True)
    if n_embeddings_per_datum > 1:
        ds = ds.repeat(n_embeddings_per_datum)
    with torch.no_grad():
        return (
            ds
            .batch(32) # TODO: Set this correctly
            .collate()
            .map(
                lambda b: get_cond_embeddings(
                    b, input_type, device, autoencoder, clip_model
                ),
            )
...
def main():
    ...
    train_dataset_new = get_condition_embeddings_2(
        args.input_type,
        args.device,
        net,
        clip_model,
        get_dataloader(args, n_px, split="train"),
        n_embeddings_per_datum=args.num_views,
    )
    train_dataloader_new = DataLoader2(
        train_dataset_new,
        # batch_size=args.batch_size,
        batch_size=None,
        shuffle=True,
    ...
    val_dataset_new = get_condition_embeddings_2(
        args.input_type,
        args.device,
        net,
        clip_model,
        get_dataloader(args, n_px, split="val"),
        n_embeddings_per_datum=1,
    )
    val_dataloader_new = DataLoader2(
        val_dataset_new,
        # batch_size=args.test_batch_size,
        batch_size=None,
        shuffle=False,
